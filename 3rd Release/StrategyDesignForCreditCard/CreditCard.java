...
public void Payment(double price){
	...
	if (sc.hasNextInt()) {
		paymentchoice = sc.nextInt();
			...
			while (loopNum&&loopLength){
				this.CardNum =sc.nextLine();
				Context context = new Context(new VerifyCCLength());
				if (context.executeStrategy(CardNum)) {
					context = new Context(new VerifyNum());
					if (context.executeStrategy(CardNum))
						loopLength =false;
					
					else {
						continue; //retries to get a new number
					}
				/*if ((CardNum.length() < 17) && ( CardNum.length() > 14)) {
					...
					} previously made verification process*/
					loopNum=false;
